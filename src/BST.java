...
public class BST {
    ...
    public class Node {
        ...
        public byte[] val;         // associated data
        private Node left, right;  // left and right subtrees
        private int size;          // number of nodes in subtree

        public Node(int key, byte[] val, int size) {
            this.key = key;
            this.val = val;
            this.size = size;
        }
    ...
    private byte[] get(Node x, int key) {
    ...
    private Node put(Node x, int key, byte[] val) {
     ...
     * use the foreach notation: {@code for (Integer key : st.keys())}.
     *
    ...
    private void keys(Node x, LinkedList<Integer> queue, int lo, int hi) {
        if (x == null) return;
        if (lo < x.key) keys(x.left, queue, lo, hi);
        if (lo <= x.key && hi >= x.key) queue.addLast(x.key);
